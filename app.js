/**
 * app.js — 개인 아카이브 웹페이지 (Firebase Firestore 연동)
 *
 * 목차
 *  1.  Firebase 초기화
 *  2.  더미 데이터 (최초 1회 DB 업로드용)
 *  3.  유틸리티
 *  4.  Firestore CRUD 헬퍼
 *  5.  관리자 인증 + 편집 모드
 *  6.  테마 (다크/라이트)
 ...
 *  8.  네비게이션 스크롤 효과
 *  9.  BGM 플레이어
 * 10.  최근 업데이트 배너 ticker
 * 11.  폴더 열기/닫기
 * 12.  영상 렌더링 + 필터 (YouTube 링크)
 * 13.  글쓰기 렌더링 + 필터 + 모달
 * 14.  독서 기록 — 책장 + 모달
 * 15.  독서 통계 바 차트
 * 16.  검색
...
/* ═══════════════════════════════════════════════════
   2. 더미 데이터 (DB가 비어있을 때 최초 1회 업로드)
════════════════════════════════════════════════════ */

const SEED_VIDEOS = [
  { id: 'v1', title: '교토에서 보낸 3일 — 골목골목 여행기', date: '2025.05.18',
    category: 'vlog', emoji: '🏯', gradient: 'linear-gradient(135deg,#fde68a,#f59e0b)',
    youtubeUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
    desc: '혼자 떠난 교토 여행. 새벽 5시 금각사부터 야시장까지 발이 닳도록 걸어다닌 3일간의 기록.',
    tags: ['여행','교토','일본','브이로그'] },
  { id: 'v2', title: '나만의 독서 루틴 — 한 달에 책 5권 읽는 법', date: '2025.04.30',
    category: 'study', emoji: '📖', gradient: 'linear-gradient(135deg,#a78bfa,#7c3aed)',
    youtubeUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
    desc: '바쁜 직장인도 실천할 수 있는 독서 습관.',
    tags: ['독서','루틴','자기계발'] },
  { id: 'v3', title: '성수동 카페 투어 — 숨겨진 공간들', date: '2025.04.12',
    category: 'vlog', emoji: '☕', gradient: 'linear-gradient(135deg,#6ee7b7,#059669)',
    youtubeUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
    desc: '성수동 메인 스트리트를 벗어나 발견한 진짜 숨은 카페들.',
    tags: ['카페','성수동','서울','브이로그'] },
  { id: 'v4', title: '「채식주의자」 북리뷰 — 한강 노벨문학상', date: '2025.03.28',
    category: 'review', emoji: '🌿', gradient: 'linear-gradient(135deg,#f9a8d4,#ec4899)',
    youtubeUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
    desc: '노벨문학상 발표 이후 다시 읽은 채식주의자.',
    tags: ['북리뷰','한강','소설'] },
  { id: 'v5', title: '나의 애플 생태계 셋업 투어 2025', date: '2025.03.10',
    category: 'review', emoji: '🖥️', gradient: 'linear-gradient(135deg,#bfdbfe,#3b82f6)',
    youtubeUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
    desc: 'M3 MacBook Air, iPad Pro, AirPods Max 실사용 후기.',
    tags: ['애플','테크','셋업','리뷰'] },
  { id: 'v6', title: '제주 한달살기 브이로그 Ep.1', date: '2025.02.14',
    category: 'vlog', emoji: '🌊', gradient: 'linear-gradient(135deg,#fed7aa,#ea580c)',
    youtubeUrl: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
    desc: '회사를 잠시 떠나 제주에서 한 달을 보낸 이야기.',
    tags: ['제주','한달살기','브이로그'] },
];

const SEED_WRITINGS = [
  { id: 'w1', title: '서울의 카페를 전부 가봤다 — 는 거짓말이고', date: '2025.05.24',
    category: 'essay', tags: ['에세이','카페','서울'],
    excerpt: '카페가 이렇게 많은 도시에 살고 있으면서 왜 나는 항상 같은 자리에 앉아 같은 아메리카노를 시킬까.',
    body: '<p>카페가 이렇게 많은 도시에 살고 있으면서 왜 나는 항상 같은 자리에 앉아 같은 아메리카노를 시킬까.</p><p>익숙한 카페로 돌아가는 건 사실 일종의 의식이다.</p>' },
  { id: 'w2', title: '혼자 있는 능력에 대하여', date: '2025.05.10',
    category: 'essay', tags: ['에세이','고독','일상'],
    excerpt: '혼자 있는 것을 즐기는 사람과, 혼자 있을 수밖에 없는 사람은 겉으로는 같아 보인다.',
    body: '<p>혼자 있는 것과 고립은 종이 한 장 차이다. 그 경계를 의식적으로 인식하는 것이 어쩌면 진짜 능력일지도.</p>' },
  { id: 'w3', title: '교토 여행에서 배운 것들', date: '2025.04.20',
    category: 'review', tags: ['후기','여행','교토'],
    excerpt: '5박 6일이라는 짧지 않은 시간 동안 교토에 있었다.',
    body: '<p>교토는 서두르면 손해보는 도시다. 이 여행에서 배운 것은 결국 느리게 보는 것의 가치였다.</p>' },
  { id: 'w4', title: '올해의 책 5권을 고른다면', date: '2025.03.30',
    category: 'review', tags: ['후기','독서','추천'],
    excerpt: '삶에 대해 다르게 생각하게 만든 책 5권.',
    body: '<p>채식주의자, 아무것도 하지 않는 시간의 힘, 82년생 김지영, 데미안, 아몬드.</p>' },
  { id: 'w5', title: '4월의 단상들', date: '2025.04.05',
    category: 'daily', tags: ['일상','일기','봄'],
    excerpt: '올해 봄은 왜인지 조금 달랐다.',
    body: '<p>4월 8일. 벚꽃이 폈다. 4월 30일. 이 달은 꽤 괜찮았다.</p>' },
  { id: 'w6', title: '계획 없이 사는 것에 대한 변명', date: '2025.02.25',
    category: 'essay', tags: ['에세이','생각','삶'],
    excerpt: '5년 계획을 물어보는 사람들에게 나는 아직도 제대로 된 대답을 못한다.',
    body: '<p>계획 없이 산다는 것은 목표가 없다는 게 아니다. 지금 이 순간이 좋은 방향으로 흐르고 있다는 감각을 신뢰하는 것.</p>' },
];

const SEED_BOOKS = [
  { id:'b1',  title:'채식주의자',             author:'한강',        date:'2025.05.20', stars:5, color:'#2d6a4f', width:24, quote:'인간이라는 사실이, 이렇게 수치스러울 수 없었다.',              memo:'두 번째로 읽었다. 예술이라는 이름으로 포장된 욕망에 대해 계속 생각하게 된다.' },
...
const SEED_MONTHLY = [
  { id:'m1', month:'1월', count:4 },
  { id:'m2', month:'2월', count:3 },
  { id:'m3', month:'3월', count:5 },
  { id:'m4', month:'4월', count:6 },
...
async function dbSet(colName, id, data) {
  const { id: _id, ...rest } = data; // id 필드 제거 후 저장
...
function setAdminMode(on) {
  ...
  if (on)  toast('✏️ 편집 모드 활성화 — 항목을 클릭해서 수정하세요');
  else     toast('🔒 편집 모드 종료');
}
...
adminToolbar.innerHTML = `
  ...
  <div class="admin-toolbar-btns">
    ...
    <button class="adm-btn" id="admAddVideo">＋ 영상 추가</button>
    <button class="adm-btn" id="admAddWriting">＋ 글 추가</button>
    <button class="adm-btn" id="admAddBook">＋ 책 추가</button>
  </div>`;
...
function openEditModal(title, fields, onSave) {
  ...
  const fieldsHtml = fields.map(f => {
    let input = '';
    if (f.type === 'textarea')
      input = `<textarea class="edit-input edit-textarea" data-key="${f.key}" rows="4">${f.value??''}</textarea>`;
    else if (f.type === 'select')
      input = `<select class="edit-input" data-key="${f.key}">${f.options.map(o=>`<option value="${o}"${o===f.value?' selected':''}>${o}</option>`).join('')}</select>`;
    else if (f.type === 'number')
      input = `<input type="number" class="edit-input" data-key="${f.key}" value="${f.value??''}" min="${f.min??0}" max="${f.max??999}">`;
    else
      input = `<input type="text" class="edit-input" data-key="${f.key}" value="${f.value??''}" placeholder="${f.placeholder??''}">`;
...
function editVideo(v) {
  openEditModal('📹 영상 수정', [
    ...
    Object.assign(v, { title:r.title, date:r.date, youtubeUrl:r.youtubeUrl,
      category:r.category, emoji:r.emoji, desc:r.desc,
...
document.getElementById('admAddVideo').addEventListener('click', () => {
  const newV = { id:'v'+uid(), title:'새 영상', date:'', category:'vlog',
    emoji:'🎬', gradient:'linear-gradient(135deg,#a0aec0,#718096)',
    youtubeUrl:'', desc:'', tags:[] };
  VIDEO_DATA.unshift(newV);
  editVideo(newV);
});

/* ── 글 편집/삭제 ── */
async function saveWriting(w) {
  await dbSet('writings', w.id, w);
  toast('💾 글 저장 완료');
  renderWritings();
}
...
function editWriting(w) {
  openEditModal('✍️ 글 수정', [
    ...
    Object.assign(w, { title:r.title, date:r.date, category:r.category,
      tags:r.tags.split(',').map(t=>t.trim()).filter(Boolean),
...
document.getElementById('admAddWriting').addEventListener('click', () => {
  const newW = { id:'w'+uid(), title:'새 글', date:'', category:'essay',
    tags:[], excerpt:'', body:'<p>내용을 입력하세요.</p>' };
  WRITING_DATA.unshift(newW);
  editWriting(newW);
});

/* ── 책 편집/삭제 ── */
async function saveBook(b) {
  await dbSet('books', b.id, b);
  toast('💾 책 저장 완료');
  renderBookshelf();
}
...
function editBook(b) {
  openEditModal('📚 책 수정', [
    ...
    Object.assign(b, { title:r.title, author:r.author, date:r.date,
      stars:Math.min(5,Math.max(1,parseInt(r.stars)||1)),
      color:r.color||'#888', quote:r.quote, memo:r.memo });
    await saveBook(b);
  });
}
...
document.getElementById('admAddBook').addEventListener('click', () => {
  ...
  const newB = { id:'b'+uid(), title:'새 책', author:'', date:'', stars:4,
    color:colors[Math.floor(Math.random()*colors.length)], width:20, quote:'', memo:'' };
  BOOK_DATA.push(newB);
  editBook(newB);
});
...
window.addEventListener('scroll', () => {
  topNav.style.boxShadow = window.scrollY > 10 ? 'var(--shadow-md)' : 'none';
}, { passive:true });
...
function loadTrack(idx) {
  const t = BGM_TRACKS[idx];
  playerTitle.textContent=t.title; playerArtist.textContent=t.artist;
  totalTimeEl.textContent=t.duration; progress=0;
  progressFill.style.width='0%'; currentTimeEl.textContent='0:00';
}

function startProgress() {
  clearInterval(progressTimer);
  const [m,s] = BGM_TRACKS[trackIdx].duration.split(':').map(Number);
  const total = m*60+s;
  progressTimer = setInterval(() => {
    progress += (1/total)*100;
    if (progress>=100) { progress=0; nextTrack(); return; }
    progressFill.style.width = progress+'%';
    const e = Math.floor((progress/100)*total);
    currentTimeEl.textContent = `${Math.floor(e/60)}:${(e%60).toString().padStart(2,'0')}`;
  }, 1000);
}
...
playBtn.addEventListener('click', () => {
  ...
  playerDisc.classList.toggle('spinning', isPlaying);
...
function togglePanel(type) {
  const panel = folderPanels[type];
  if (!panel) return;
  const isOpen = panel.classList.contains('open');
  Object.values(folderPanels).forEach(p=>hide(p));
  if (!isOpen) { show(panel); panel.scrollIntoView({behavior:'smooth',block:'start'}); }
}
...
document.getElementById('folderBooks').addEventListener('click', ()=>{
  document.getElementById('bookshelf').scrollIntoView({behavior:'smooth',block:'start'});
});
    ...
    <div class="video-card" data-id="${v.id}" data-category="${v.category}">
      ...
      <a href="${v.youtubeUrl||'#'}" target="_blank" rel="noopener" class="video-card-link">
        <div class="video-thumb">
          <div class="video-thumb-bg" style="background:${v.gradient}">
...
function renderVideos(filter='all') {
  ...
  videoGrid.querySelectorAll('.admin-edit-btn').forEach(btn=>{
    btn.addEventListener('click', e=>{ e.stopPropagation(); e.preventDefault();
      const v=VIDEO_DATA.find(x=>x.id===btn.dataset.vid); if(v)editVideo(v); });
  });
  videoGrid.querySelectorAll('.admin-del-btn').forEach(btn=>{
    btn.addEventListener('click', e=>{ e.stopPropagation(); e.preventDefault();
      deleteVideo(btn.dataset.vid); });
  });
}
...
function renderWritings(filter='all') {
  ...
  writingList.querySelectorAll('.admin-edit-btn').forEach(btn=>{
    btn.addEventListener('click', e=>{ e.stopPropagation();
      const w=WRITING_DATA.find(x=>x.id===btn.dataset.wid); if(w)editWriting(w); });
  });
  writingList.querySelectorAll('.admin-del-btn').forEach(btn=>{
    btn.addEventListener('click', e=>{ e.stopPropagation(); deleteWriting(btn.dataset.wid); });
  });
}
...
function createBookSpine(b) {
  return `<div class="book-spine" data-id="${b.id}" style="background:${b.color};width:${b.width}px;">
...
function renderBookshelf() {
  const perRow=8;
  let html='';
  for(let i=0;i<BOOK_DATA.length;i+=perRow){
    html+=`<div class="shelf-row">${BOOK_DATA.slice(i,i+perRow).map(createBookSpine).join('')}</div>`;
  }
  bookshelfEl.innerHTML=html;
  bookshelfEl.querySelectorAll('.book-spine').forEach(spine=>{
    spine.addEventListener('click', ()=>{
      if(isAdminMode) showBookAdminMenu(BOOK_DATA.find(x=>x.id===spine.dataset.id), spine);
      else openBookModal(spine.dataset.id);
    });
  });
}
...
function openBookModal(id) {
  ...
  const coverEl=document.getElementById('bookModalCover');
  coverEl.style.background=`linear-gradient(135deg,${b.color},color-mix(in srgb,${b.color} 70%,#000))`;
  coverEl.innerHTML=`<div style="text-align:center;color:rgba(255,255,255,.9);padding:20px;"><div style="font-size:2.5rem;margin-bottom:12px;">📖</div><p style="font-size:.8rem;line-height:1.4;font-family:'Noto Serif KR',serif;">${b.title}</p><p style="font-size:.65rem;opacity:.7;margin-top:6px;">${b.author}</p></div>`;
...
function renderBarChart() {
  ...
  const max = Math.max(...MONTHLY_DATA.map(d=>d.count));
  barChartEl.innerHTML = MONTHLY_DATA.map(d=>`
    <div class="bar-col">
      <div class="bar-fill" style="height:${(d.count/max)*100}%" title="${d.month}: ${d.count}권"></div>
...
function performSearch(query) {
  if(!query.trim()){ hide(searchResults); return; }
  const q=query.toLowerCase(), results=[];
  VIDEO_DATA.forEach(v=>{ if(v.title.toLowerCase().includes(q)||v.tags.some(t=>t.toLowerCase().includes(q))) results.push({type:'영상',title:v.title,id:v.id,kind:'video'}); });
  WRITING_DATA.forEach(w=>{ if(w.title.toLowerCase().includes(q)||w.tags.some(t=>t.toLowerCase().includes(q))) results.push({type:'글',title:w.title,id:w.id,kind:'writing'}); });
  BOOK_DATA.forEach(b=>{ if(b.title.toLowerCase().includes(q)||b.author.toLowerCase().includes(q)) results.push({type:'책',title:`${b.title} — ${b.author}`,id:b.id,kind:'book'}); });

  searchResultsList.innerHTML = results.length===0
    ? '<p style="font-size:.82rem;color:var(--text-faint);padding:8px 0;">검색 결과가 없습니다.</p>'
    : results.map(r=>`<div class="search-result-item" data-kind="${r.kind}" data-id="${r.id}"><span class="search-result-type">${r.type}</span><span class="search-result-title">${r.title}</span></div>`).join('');
...
async function init() {
  ...
  try {
    ...
    await Promise.all([
      ...
      seedIfEmpty('monthly',  SEED_MONTHLY),
    ]);
    ...
    toast('❌ 데이터 로드 실패. 새로고침 해주세요.', 'error');
  } finally {
    hideLoading();
  }
}
...
const DEFAULT_PROFILE = {
  name:        '김아라',
  nameEn:      'Ara Kim',
  bio:         '글 쓰고 책 읽고 영상 만드는 사람 ✦ 기록으로 존재를 증명하는 중',
  statusEmoji: '📖',
  statusMsg:   '요즘은 「채식주의자」 를 다시 읽고 있어요',
  statusBadge: '독서 중',
};
...
function applyProfile(p) {
  ...
  if (nameEl) nameEl.childNodes[0].textContent = p.name + ' ';
  const nameEnEl = document.querySelector('.name-en');
  if (nameEnEl) nameEnEl.textContent = p.nameEn;
  // 소개
  const bioEl = document.querySelector('.profile-bio');
  if (bioEl) bioEl.textContent = p.bio;
  // 상태 배지
  const badgeEl = document.querySelector('.status-text');
  if (badgeEl) badgeEl.textContent = p.statusBadge;
  // 상태 메시지
  const emojiEl = document.querySelector('.status-emoji');
  if (emojiEl) emojiEl.textContent = p.statusEmoji;
  const msgEl = document.querySelector('.status-msg');
  if (msgEl) msgEl.textContent = p.statusMsg;
}

/** SNS DOM에 반영 */
function applySns(s) {
  const btns = document.querySelectorAll('.sns-btn');
  btns.forEach(btn => {
    const label = btn.getAttribute('aria-label');
    if (label === 'Instagram' && s.instagram) btn.href = s.instagram;
    if (label === 'YouTube'   && s.youtube)   btn.href = s.youtube;
    if (label === 'Brunch'    && s.brunch)     btn.href = s.brunch;
    if (label === 'GitHub'    && s.github)     btn.href = s.github;
  });
}

/** Firestore에서 SNS 불러오기 */
async function loadSns() {
  try {
    const snap = await getDocs(collection(db, 'sns'));
    if (!snap.empty) {
      applySns(snap.docs[0].data());
    } else {
      await dbSet('sns', 'main', DEFAULT_SNS);
      applySns(DEFAULT_SNS);
    }
  } catch(e) { console.error('SNS 로드 실패', e); }
}
...
document.getElementById('admEditProfile').addEventListener('click', async () => {
  ...
  openEditModal('👤 프로필 수정', [
    ...
    { label: '상태 배지 텍스트',  key: 'statusBadge', type: 'text',     value: badge,   placeholder: '독서 중' },
    { label: '상태 이모지',       key: 'statusEmoji', type: 'text',     value: emoji,   placeholder: '📖' },
    { label: '상태 메시지',       key: 'statusMsg',   type: 'textarea', value: msg },
  ], async (r) => {
    const newProfile = {
      name: r.name, nameEn: r.nameEn, bio: r.bio,
      statusBadge: r.statusBadge, statusEmoji: r.statusEmoji, statusMsg: r.statusMsg,
    };
    await dbSet('profile', 'main', newProfile);
    applyProfile(newProfile);
    toast('💾 프로필 저장 완료 — 모든 방문자에게 반영됩니다');
  });
});
...
document.getElementById('admEditSns').addEventListener('click', async () => {
  ...
  btns.forEach(btn => {
    const label = btn.getAttribute('aria-label');
    if (label==='Instagram') ig = btn.href === location.href ? '' : btn.href;
    if (label==='YouTube')   yt = btn.href === location.href ? '' : btn.href;
    if (label==='Brunch')    br = btn.href === location.href ? '' : btn.href;
    if (label==='GitHub')    gh = btn.href === location.href ? '' : btn.href;
  });
  ...
  openEditModal('🔗 SNS 링크 수정', [
    ...
    const newSns = { instagram: r.instagram, youtube: r.youtube, brunch: r.brunch, github: r.github };
    await dbSet('sns', 'main', newSns);
    applySns(newSns);
    toast('💾 SNS 링크 저장 완료');
  });
});

// 페이지 로드 시 프로필 + SNS 불러오기 (init 함수에서 호출)
// → init() 내부에 추가되도록 아래에서 패치
const _origInit = init;
// init 함수를 재정의해서 프로필/SNS 로드 추가
async function initWithProfile() {
  await Promise.all([loadProfile(), loadSns()]);
}
